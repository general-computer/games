...
function performPlayerAction(action, playerId, amount = 0) {
    const player = players[playerId];
    
    switch (action) {
      case 'bet':
        if (amount >= player.stack) {
          performPlayerAction('all-in', playerId);
        } else {
          player.stack -= amount;
          player.bet += amount;
          pot += amount;
          updatePlayerStack(playerId);
          updatePlayerBet(playerId);
          updatePot();
        }
        break;
      ...
      case 'fold':
        ...
        const foldedElement = document.getElementById(`player${playerId}-folded`);
